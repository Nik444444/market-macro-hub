...
export default function App(){
  const [keys,setKeys]=useState(()=>({groq:localStorage.getItem(LS.groq)||DEFAULT_KEYS.groq,finnhub:localStorage.getItem(LS.finnhub)||DEFAULT_KEYS.finnhub,model:localStorage.getItem(LS.model)||DEFAULT_KEYS.model,autoAi:localStorage.getItem(LS.autoAi)!=='false'}))
  ...
  const runAi = useCallback(async(assetId, n=news, c=calendar)=>{
    if(!keys.groq || !keys.autoAi) return
    setAiStatus('AI сжимает macro flow')
    const assetNews = filterNews(n, assetId).slice(0,5)
    const importantCal = c.filter(x=>impactOf(x)!=='low').slice(0,8)
    try{
      const result=await groqAnalyze(keys.groq,keys.model,{asset:assetId, news:assetNews.map(x=>({h:x.headline,s:shortText(x.summary,90),src:x.source,impact:newsImpact(x)})), calendar:importantCal.map(x=>({c:x.country,e:x.event,i:impactOf(x),est:x.estimate,prev:x.previous,t:x.date}))})
  ...
  },[keys.groq,keys.model,keys.autoAi,news,calendar])
  const refresh=useCallback(async(forceAi=false)=>{ setBusy(true); setErr(''); try{ const [n,c]=await Promise.all([fetchFinnhubNews(keys.finnhub),fetchFinnhubCalendar(keys.finnhub)]); setNews(n); setCalendar(c); setUpdated(new Date()); const shouldAi=keys.groq && keys.autoAi && (forceAi || Date.now()-lastAiRef.current>5*60*1000); if(shouldAi) await runAi(selected,n,c); else if(!keys.groq){ setAi(null); setAiAsset(null); setAiStatus('локальный анализ') } } catch(e){ setErr(e.message) } finally{ setBusy(false); setLoading(false)} },[keys.finnhub,keys.groq,keys.autoAi,selected,runAi])
  useEffect(()=>{ refresh(true); const t=setInterval(()=>refresh(false),60000); return()=>clearInterval(t) },[refresh])
  useEffect(()=>{ setAi(null); setAiAsset(null); if(news.length&&calendar.length&&keys.groq&&keys.autoAi){ const t=setTimeout(()=>runAi(selected,news,calendar),180); return()=>clearTimeout(t) } },[selected])
  ...
  return <div className="app">
    ...
    <main className="workspace">
      ...
      <section className="panel settings-panel"><div className="panel-head"><h2>API кабинет</h2><span>{DEFAULT_KEYS.finnhub||DEFAULT_KEYS.groq?'default keys активны':'можно вставить свои ключи'}</span></div><label>Finnhub API key<input value={keys.finnhub} onChange={e=>save('finnhub',e.target.value)} placeholder="Finnhub key"/></label><label>Groq API key<input value={keys.groq} onChange={e=>save('groq',e.target.value)} placeholder="Groq key"/></label><label>Groq model<input value={keys.model} onChange={e=>save('model',e.target.value)}/></label><label className="toggle"><input type="checkbox" checked={keys.autoAi} onChange={e=>save('autoAi',e.target.checked)}/><span>Авто AI трактовка каждые 5 минут</span></label><button className="refresh" onClick={()=>refresh(true)}>Обновить сейчас</button>{err&&<div className="err">{err}</div>}</section>
