...
const ASSETS = [
  { id:'ALL', label:'MARKET', emoji:'🌐', queries:['forex','federal reserve','ecb','inflation','jobs','dollar','gold','treasury','risk'] },
  { id:'EURUSD', label:'EUR/USD', emoji:'🇪🇺', queries:['euro','ecb','eurusd','eurozone','dollar','lagarde','germany'] },
  { id:'GBPUSD', label:'GBP/USD', emoji:'🇬🇧', queries:['pound','boe','gbpusd','uk inflation','dollar','bailey'] },
  { id:'XAUUSD', label:'XAU/USD', emoji:'🥇', queries:['gold','xau','treasury yields','real yields','dollar','safe haven'] },
  { id:'INDICES', label:'INDEX', emoji:'🇺🇸', queries:['nasdaq','sp500','risk sentiment','stocks','federal reserve','yields'] },
]
...
function localMacro(news, calendar){
  const text=[...news.map(n=>`${n.headline} ${n.summary||''}`),...calendar.map(c=>`${c.country} ${c.event}`)].join(' ').toLowerCase()
  ...
  const primary=high[0]?.country==='USD'?'USD calendar':high[0]?.country==='EUR'?'EUR calendar':high[0]?.event||'headline flow'
  ...
  return {
    ...
    assetBias:[
      ...
      {asset:'INDEX',bias:idxBias,reason:idxBias==='LONG'?'Risk-on поддерживает индексы.':'Перед macro событием лучше ждать факта.',trigger:'мягкие данные + USD/yields вниз',risk:'hawkish surprise'}
    ],
...
async function groqAnalyze(key, model, payload){
  ...
  const prompt=`Ты институциональный макро-аналитик и ICT intraday trader. На основе новостей и календаря дай готовый dashboard для трейдера. НЕ выдумывай факты, используй только входные данные. Верни строго JSON на русском:
{"macroBias":"RISK_ON/RISK_OFF/EVENT_RISK/NEUTRAL","dollarBias":"BULLISH/BEARISH/NEUTRAL","primaryAsset":"главный драйвер","confidence":0-100,"headline":"главная идея дня","drivers":["4 коротких причины"],"regime":{"label":"NEWS DRIVEN/RISK ON/RISK OFF/BALANCED","meaning":"что это значит для трейдера"},"assetBias":[{"asset":"EUR/USD","bias":"LONG/SHORT/WAIT","reason":"почему","trigger":"условие активации сетапа","risk":"что отменяет"}],"riskEvents":[{"time":"HH:MM","event":"название","impact":"HIGH/MEDIUM/LOW","meaning":"как трактовать"}],"executionPlan":{"beforeNews":"что делать до новости","afterNews":"что делать после новости","avoid":"когда не торговать"},"scenarios":[{"name":"сценарий","prob":0-100,"play":"что делать"}],"ictNarrative":"развернутый план 6-10 строк без воды"}`
  const res=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${key}`},body:JSON.stringify({model:model||'llama-3.1-8b-instant',temperature:.18,max_tokens:1800,messages:[{role:'system',content:prompt},{role:'user',content:JSON.stringify(payload).slice(0,14000)}]})})
...
function Loading(){return <div className="boot"><div className="orb"/><div className="radar"><i/><i/><i/></div><h1>MARKET DEVILS</h1><p>собираю macro bias · risk events · execution protocol</p><div className="loader"><i/></div></div>}
...
export default function App(){
  ...
  const macro=useMemo(()=>ai||localMacro(news,calendar),[ai,news,calendar])
  ...
  const refresh=useCallback(async(forceAi=false)=>{ setBusy(true); setErr(''); try{ const [n,c]=await Promise.all([fetchFinnhubNews(keys.finnhub),fetchFinnhubCalendar(keys.finnhub)]); setNews(n); setCalendar(c); setUpdated(new Date()); const shouldAi=keys.groq && keys.autoAi && (forceAi || Date.now()-lastAiRef.current>5*60*1000); if(shouldAi){ setAiStatus('AI анализирует поток'); try{ const result=await groqAnalyze(keys.groq,keys.model,{selected,news:n.slice(0,20),calendar:c.slice(0,30)}); setAi(result); lastAiRef.current=Date.now(); setAiStatus('AI трактовка активна') }catch(e){ setErr(e.message); setAiStatus('локальный режим'); setAi(null) } } else if(!keys.groq){ setAi(null); setAiStatus('локальный режим') } } catch(e){ setErr(e.message) } finally{ setBusy(false); setLoading(false)} },[keys.finnhub,keys.groq,keys.model,keys.autoAi,selected])
  ...
  return <div className="app">
    ...
    <main className="workspace">
      ...
      <section className="panel macro-panel"><div className="panel-head"><h2>Новости + макро поток</h2><span>{visibleNews.length} заголовков</span></div><div className="news-stack">{visibleNews.map((n,i)=><NewsCard key={n.id||i} n={n}/>)}</div></section>
