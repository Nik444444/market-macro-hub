...
const AI_CACHE_KEY = 'md_ai_cache_v20_stable_pro'
...
function localMacro(news, calendar, selected='ALL'){
  ...
  const macroScore = [
    {name:'USD', score:clamp(usd*2 + high.filter(c=>String(c.country).includes('USD')).length*3 - (text.includes('weak dollar')?4:0), -10, 10), note:'ставки · инфляция · доходности'},
    {name:'EUR', score:clamp(eur*2 + high.filter(c=>String(c.country).includes('EUR')).length*2 - (dollarBias==='BULLISH'?3:0), -10, 10), note:'ECB · еврозона'},
    {name:'GBP', score:clamp(gbp*2 + high.filter(c=>String(c.country).includes('GBP')).length*2 - (dollarBias==='BULLISH'?3:0), -10, 10), note:'BoE · UK data'},
    {name:'GOLD', score:clamp(gold*2 + (macroBias==='RISK_OFF'?3:0) - (dollarBias==='BULLISH'?4:0), -10, 10), note:'safe haven · real yields'},
    {name:'RISK', score:clamp(risk*2 + (macroBias==='RISK_ON'?3:0) - high.length, -10, 10), note:'индексы · risk appetite'}
  ]
  const tradeability = [
    {asset:'EUR/USD',score:clamp(5 + Math.abs((eur*2)-(usd*2)) + high.filter(c=>['USD','EUR'].some(x=>String(c.country).includes(x))).length, 1, 10), reason:'EUR vs USD catalyst'},
    {asset:'GBP/USD',score:clamp(5 + Math.abs((gbp*2)-(usd*2)) + high.filter(c=>['USD','GBP'].some(x=>String(c.country).includes(x))).length, 1, 10), reason:'GBP vs USD catalyst'},
    {asset:'XAU/USD',score:clamp(5 + gold + usd + high.filter(c=>String(c.country).includes('USD')).length, 1, 10), reason:'USD/yields + risk'},
    {asset:'NASDAQ',score:clamp(4 + risk + usd + (macroBias==='EVENT_RISK'?2:0), 1, 10), reason:'risk + yields'},
    {asset:'S&P 500',score:clamp(4 + risk + (macroBias==='EVENT_RISK'?2:0), 1, 10), reason:'broad risk'},
    {asset:'GER40',score:clamp(4 + eur + risk + high.filter(c=>String(c.country).includes('EUR')).length, 1, 10), reason:'Europe + ECB'}
  ].sort((a,b)=>b.score-a.score)
  const nextCatalyst = high[0] ? {time:timeFmt(high[0].date), event:high[0].event, country:high[0].country, impact:'HIGH', meaning:eventMeaning(high[0]), affected:['USD','GOLD','INDICES','EUR','GBP'].filter(x => text.includes(x.toLowerCase()) || String(high[0].country).includes(x.slice(0,3)) || x==='USD')} : {time:'—', event:'Нет high-impact в ближайшем окне', country:'MARKET', impact:'LOW', meaning:'Работаем от headline-flow и реакции USD/yields.', affected:['ALL']}
  const marketMemory = {time: timeFmt(Date.now()), change: `${macroBias} / USD ${dollarBias}`, reason: high[0] ? `фокус на ${high[0].country} ${high[0].event}` : 'обновление headline-flow'}
  return {
    macroBias,dollarBias,confidence,primaryAsset:primary, macroScore, tradeability, nextCatalyst, marketMemory,
...
{"macroBias":"RISK_ON/RISK_OFF/EVENT_RISK/NEUTRAL","dollarBias":"BULLISH/BEARISH/NEUTRAL","primaryAsset":"driver","confidence":0-100,"headline":"1 фраза","drivers":["4 коротких пункта"],"regime":{"label":"NEWS DRIVEN/RISK ON/RISK OFF/BALANCED","meaning":"1 фраза"},"assetBias":[{"asset":"EUR/USD","bias":"LONG/SHORT/WAIT","reason":"1 фраза","trigger":"условие","risk":"отмена"}],"executionPlan":{"beforeNews":"до","afterNews":"после","avoid":"не торговать"},"scenarios":[{"name":"сценарий","prob":0-100,"play":"действие"}],"ictNarrative":"4-5 коротких строк","macroScore":[{"name":"USD","score":0,"note":""}],"tradeability":[{"asset":"EUR/USD","score":0,"reason":""}],"nextCatalyst":{"time":"","event":"","impact":"","meaning":""},"marketMemory":{"time":"","change":"","reason":""}}`
  const smallPayload = JSON.stringify(payload).slice(0,900)
  const res=await fetch('https://api.groq.com/openai/v1/chat/completions',{method:'POST',headers:{'Content-Type':'application/json','Authorization':`Bearer ${key}`},body:JSON.stringify({model:model||'llama-3.1-8b-instant',temperature:.12,max_tokens:360,messages:[{role:'system',content:prompt},{role:'user',content:smallPayload}]})})
...
function ScoreBoard({macro}){return <div className="score-board">
  <div className="score-section"><div className="mini-title">Macro Score</div>{macro.macroScore?.map((x,i)=><div className="score-row" key={i}><span>{x.name}</span><b className={x.score>2?'long':x.score<-2?'short':'wait'}>{x.score>0?`+${x.score}`:x.score}</b><em>{x.note}</em></div>)}</div>
  <div className="score-section"><div className="mini-title">Tradeability</div>{macro.tradeability?.slice(0,6).map((x,i)=><div className="score-row" key={i}><span>{x.asset}</span><b>{Number(x.score).toFixed(1)}/10</b><em>{x.reason}</em></div>)}</div>
  <div className="score-section catalyst"><div className="mini-title">Next Catalyst</div><strong>{macro.nextCatalyst?.time} · {macro.nextCatalyst?.country}</strong><p>{macro.nextCatalyst?.event}</p><em>{macro.nextCatalyst?.meaning}</em></div>
  <div className="score-section memory"><div className="mini-title">Market Memory</div><strong>{macro.marketMemory?.time}</strong><p>{macro.marketMemory?.change}</p><em>{macro.marketMemory?.reason}</em></div>
</div>}
function AssetPlan({macro, selected}){ const item = macro.assetBias?.find(a => a.asset?.replace('/','').toUpperCase().includes(selected.replace('SP500','SP').replace('NASDAQ','NASDAQ')) || a.asset===selected); if(!item) return null; const dir=item.bias==='LONG'?'лонг':item.bias==='SHORT'?'шорт':'ожидание'; return <div className="asset-plan"><div className="mini-title">Сетап дня · {item.asset}</div><div className="setup-grid"><KeyBox label="Bias" value={item.bias} tone={item.bias?.toLowerCase()}/><KeyBox label="Откуда ждать реакцию" value={item.trigger}/><KeyBox label="Инвалидация" value={item.risk}/><KeyBox label="Режим" value={dir}/></div><p><b>План:</b> {item.reason} Сначала ждем снятие ближайшей ликвидности, затем подтверждение закреплением. Без подтверждения — пропуск.</p></div> }
...
export default function App(){
  ...
  const cached = aiCache[selected]?.data
  const macro=useMemo(()=>cached ? {...local,...cached} : local,[cached,local])
  ...
  const shouldRefreshAi = useCallback((assetId)=>{
    ...
    const cachedAt = aiCache[assetId]?.ts || 0
  ...
  const runAiFor = useCallback(async(assetId, n=news, c=calendar, {force=false}={})=>{
    ...
    const assetNews = filterNews(n, assetId).slice(0,3)
    const importantCal = c.filter(x=>impactOf(x)!=='low').slice(0,4)
    ...
    try{
      ...
      else { setAiStatus('AI недоступен — показан локальный анализ') }
  ...
  const selectAsset = useCallback((id)=>{
    setSelected(id)
    setErr('')
    setAiStatus(aiCache[id] ? 'сохранённая AI трактовка' : 'локальный анализ')
    clearTimeout(debounceRef.current)
    debounceRef.current=setTimeout(()=>{
      if(news.length && calendar.length) runAiFor(id,news,calendar)
    }, 2200)
  },[aiCache, news, calendar, runAiFor])
  ...
  const refreshData = useCallback(async({forceAi=false, silent=false}={})=>{
    ...
    try{
      ...
      setNews(n); setCalendar(c); const ts=new Date(); setUpdated(ts); setNextUpdate(new Date(ts.getTime()+5*60_000))
      const current = selectedRef.current
      if(forceAi) runAiFor(current,n,c,{force:true})
      else if(shouldRefreshAi(current)) runAiFor(current,n,c)
  ...
  useEffect(()=>{
    ...
    const t=setInterval(()=>refreshData({silent:true}),5*60_000)
  ...
  return <div className="app">
    <header className="topbar"><div className="brand"><span>◈</span><b>MARKET DEVILS</b><em>MACRO OS</em></div><div className="top-status"><span>{busy?'SYNC':'ONLINE'}</span><b>обн. {updated?timeFmt(updated):'--:--'}</b><em>след. {nextUpdate?timeFmt(nextUpdate):'--:--'} · {aiStatus}</em><button className="mini-refresh" onClick={()=>refreshData({forceAi:true})}>↻</button></div></header>
    <section className="hero terminal-card"><div className="hero-left"><div className="asset-tabs asset-grid">{ASSETS.map(a=><AssetTab key={a.id} asset={a} active={selected===a.id} macro={macro} onClick={()=>selectAsset(a.id)}/>)}</div><h1>{macro.headline}</h1><p>{macro.ictNarrative}</p><div className="driver-list">{macro.drivers?.map((d,i)=><span key={i}>{d}</span>)}</div></div><MarketPulse macro={macro}/></section>
    <main className="workspace no-settings">
      <section className="panel wide"><div className="panel-head"><h2>{selected==='ALL'?'Market Intelligence':'Активный план'}</h2><span>последнее обновление {updated?timeFmt(updated):'--:--'} · следующее {nextUpdate?timeFmt(nextUpdate):'--:--'}</span></div>{selected==='ALL'?<><ScoreBoard macro={macro}/><div className="bias-grid">{macro.assetBias?.map((a,i)=><BiasCard key={i} a={a}/>)}</div></>:<AssetPlan macro={macro} selected={selected}/>}</section>
